...
export default function NewReposicionButton() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/reposicion", {
        ...
        body: JSON.stringify({ name, meses }),
      });
      if (!res.ok) {
        const d = await res.json().catch(() => ({}));
        setError(d.error ?? "No se pudo crear el análisis");
        setLoading(false);
        return;
      }
      ...
      setName("");
      setMeses("4");
      router.push(`/reposicion/${created.id}`);
      router.refresh();
    } catch {
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="animate-in card relative w-full max-w-md border-white/10 p-6 shadow-2xl">
            <h2 className="mb-1 text-lg font-bold text-white">Nuevo análisis de reposición</h2>
            <p className="mb-5 text-sm text-zinc-400">
              Creá el análisis y luego subí los Excels de ventas y stock.
            </p>
            <form onSubmit={onSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ej: Reposición mayo 2026"
                  className="field"
                  required
                  autoFocus
                />
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-zinc-500">
                  La reposición sugerida = ventas del mes × meses − stock disponible.
                </p>
              </div>
